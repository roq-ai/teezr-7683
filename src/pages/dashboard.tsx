...
import { useRoqPlatformUserProfiles, useGuestCount, useOfferCount, useDealCount } from 'lib/roq';

import { MarketingCampaignListPage as ShowCaseList } from 'pages/marketing-campaigns';

function DashboardPage() {
  const { session } = useSession();

  const guestStat = useGuestCount();

  const offerStat = useOfferCount();

  const dealStat = useDealCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Guests',
      result: guestStat?.data,
      isLoading: guestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Offers',
      result: offerStat?.data,
      isLoading: offerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Deals',
      result: dealStat?.data,
      isLoading: dealStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`marketing-campaigns`} passHref legacyBehavior>
                  <Button
