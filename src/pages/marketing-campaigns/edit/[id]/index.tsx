...
import { useRoqClient, useMarketingCampaignFindFirst } from 'lib/roq';
...
function MarketingCampaignEditPage() {
  ...
  const { data, error, isLoading, mutate } = useMarketingCampaignFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: MarketingCampaignInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.marketing_campaign.update({
        data: values as RoqTypes.marketing_campaign,
        where: {
          id,
        },
      });
