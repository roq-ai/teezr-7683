...
import { useRoqClient, useDealFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<DealInterface, unknown>;
...
export function DealListPage(props: DealListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useDealFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.deal.delete({ where: { id } });
      await mutate();
    } catch (error) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/deals/view/${record.id}`} passHref legacyBehavior>
                <Button
