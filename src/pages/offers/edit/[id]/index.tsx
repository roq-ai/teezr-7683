...
import { useRoqClient, useOfferFindFirst } from 'lib/roq';
...
function OfferEditPage() {
  ...
  const { data, error, isLoading, mutate } = useOfferFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: OfferInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.offer.update({
        data: values as RoqTypes.offer,
        where: {
          id,
        },
      });
