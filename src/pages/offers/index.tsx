...
import { useRoqClient, useOfferFindManyWithCount } from 'lib/roq';
...
export function OfferListPage(props: OfferListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useOfferFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.offer.delete({ where: { id } });
      await mutate();
    } catch (error) {
