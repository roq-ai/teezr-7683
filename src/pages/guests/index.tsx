...
import { useRoqClient, useGuestFindManyWithCount } from 'lib/roq';
...
export function GuestListPage(props: GuestListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['username.contains', 'email.contains', 'contact_number.contains'],
          ...params.filters,
        },
        'guest',
      ),
  ...
  const { data, error, isLoading, mutate } = useGuestFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.guest.delete({ where: { id } });
      await mutate();
    } catch (error) {
